    ...
    stages {
        ...
        stage('trivy scan') {
            steps {
                sh '''
                      echo "docker build..."
                       docker run --rm -v /var/run/docker.sock:/var/run/docker.sock aquasec/trivy:0.54.1 image nexus.winters-tek.net:8083/harbinman/mynode:${BUILD_NUMBER} >> result_${BUILD_NUMBER}.txt
